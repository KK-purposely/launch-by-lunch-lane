...
import { ArrowRight, Users, User, Wrench, Building2, CheckCircle2, Zap, Laptop } from "lucide-react";
...
const buildItems = [
  "A context-rich Chief of Staff trained on your goals, responsibilities, and decision-making criteria",
  "A connected workflow integrating your email, calendar, docs, CRM, and AI notetakers",
  "A daily executive briefing that pulls tasks and commitments automatically",
  "A clean, consolidated AI-news digest so you stop drowning in information",
  "A working system you can refine and expand immediately",
];
    ...
    <div className="bg-white min-h-screen">
          ...
          <div className="space-y-5 max-w-3xl mx-auto">
            {buildItems.map((item, i) => (
              <div key={i} className="flex items-start gap-4 bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-launch-purple to-launch-orange rounded-lg flex items-center justify-center text-white font-bold text-sm">
                  {i + 1}
                </div>
            ...
            return (
              ...
              >
                ...
                <CardContent className="flex flex-col flex-1 p-6 pt-8">
                  ...
                  <p className="text-sm text-gray-500 mb-4">{w.subtitle}</p>
                  <div className="mb-4">
                    <span className="text-3xl font-bold text-launch-purple">{w.price}</span>
                    {w.unit && <span className="text-sm text-gray-500 ml-1">{w.unit}</span>}
                  </div>
                  <p className="text-gray-600 text-sm flex-1 mb-6">{w.description}</p>
                  <Button
      ...
      <section className="bg-gradient-to-r from-launch-purple to-launch-orange py-14">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center text-white">
          <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-2 mb-6 text-sm font-bold uppercase tracking-wide">
            🎁 Bonus Included
          </div>
